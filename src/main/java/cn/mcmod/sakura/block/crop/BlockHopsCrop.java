...
public class BlockHopsCrop extends BlockBase implements IPlantable, IGrowable, IShearable {
		...
		if (age >= getMaxAge()) {
			for (int i = 0; i < 3 + fortune; ++i) {
				if (rand.nextInt(2 * getMaxAge()) <= age)
					drops.add(new ItemStack(ItemLoader.MATERIAL, 1, 37));
			}
		}
	}
	...
	public void dropBlockAsItemWithChance(World worldIn, BlockPos pos, IBlockState state, float chance, int fortune) {
		if (!worldIn.isRemote) {
			spawnAsEntity(worldIn, pos, new ItemStack(Item.getItemFromBlock(BlockLoader.GRAPE_SPLINT_STAND)));
			int i = this.getAge(state);
			if (i >= this.getMaxAge()) {
				int j = 3 + fortune;
				for (int k = 0; k < j; ++k) {
					if (worldIn.rand.nextInt(2 * this.getMaxAge()) <= i)
						spawnAsEntity(worldIn, pos, new ItemStack(ItemLoader.MATERIAL, 1, 37));
				}
			}
		}
	}
	...
	public List<ItemStack> onSheared(ItemStack item, IBlockAccess world, BlockPos pos, int fortune) {
		...
		if (this.isMaxAge(world.getBlockState(pos)))
			list.add(new ItemStack(ItemLoader.MATERIAL, 1 + fortune, 37));
		return list;
	}
	...
	public boolean canSustainPlant(IBlockState state, IBlockAccess world, BlockPos pos, EnumFacing direction,
		...
		return state.getMaterial() == Material.GROUND || state.getMaterial() == Material.GRASS
				|| (state.getBlock() instanceof BlockHopsCrop && this.getAge(state) >= 2)
