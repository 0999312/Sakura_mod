...
public class BlockPepperCrop extends BlockBase implements IPlantable, IGrowable, IShearable {
	...
	public void dropBlockAsItemWithChance(World worldIn, BlockPos pos, IBlockState state, float chance, int fortune) {
		if (!worldIn.isRemote) {
			spawnAsEntity(worldIn, pos, new ItemStack(Item.getItemFromBlock(BlockLoader.PEPPER_SPLINT)));
			int i = this.getAge(state);
			if (i >= this.getMaxAge()) {
				int j = 3 + fortune;
				for (int k = 0; k < j; ++k) {
	...
	public List<ItemStack> onSheared(ItemStack item, IBlockAccess world, BlockPos pos, int fortune) {
		...
		if (i == 6) {
			list.clear();
			list.add(new ItemStack(ItemLoader.MATERIAL, 1 + fortune, 18));
		} else if (i == 7) {
			list.clear();
			list.add(new ItemStack(ItemLoader.MATERIAL, 1 + fortune, 19));
		}
		return list;
	}
