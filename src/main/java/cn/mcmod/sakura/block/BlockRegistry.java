...
public class BlockRegistry {
    ...
    public static final RegistryObject<RotatedPillarBlock> BAMBOO_CHARCOAL_BLOCK = BLOCKS
            .register("bamboo_charcoal_block", () -> simplebambooBlock(MaterialColor.COLOR_GRAY, MaterialColor.COLOR_BLACK));
    ...
    private static SaplingBlock sapling(AbstractTreeGrower tree) {
        return new SaplingBlock(tree, BlockBehaviour.Properties.of(Material.PLANT).noCollission().randomTicks().instabreak().sound(SoundType.GRASS));
    }

    private static RotatedPillarBlock simplebambooBlock(MaterialColor top, MaterialColor bark) {
        return new RotatedPillarBlock(BlockBehaviour.Properties
                .of(Material.BAMBOO,state -> (state.getValue(RotatedPillarBlock.AXIS) == Direction.Axis.Y ? top : bark))
                .strength(2.0F).sound(SoundType.BAMBOO));
    }

    private static Block plank(MaterialColor material_color) {
        return new Block(BlockBehaviour.Properties.of(Material.WOOD, material_color).strength(2.0F, 3.0F).sound(SoundType.WOOD));
    }
