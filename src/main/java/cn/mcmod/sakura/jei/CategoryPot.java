...
public class CategoryPot extends BlankRecipeCategory<IRecipeWrapper>{
	...
	public void setRecipe(IRecipeLayout arg0, IRecipeWrapper arg1, IIngredients arg2) {
		...
		items.init(0, true, 37, 3);
		int k,l;
	    for (k = 1; k <5; ++k)
	        items.init(k,true, 10 + (k-1) * 18, 21);
	    for (l = 5; l <9; ++l)
        	items.init(l,true, 10 + (l-5) * 18, 39);
		...
		if(arg2.getInputs(VanillaTypes.FLUID).get(0).get(0).amount>0)
